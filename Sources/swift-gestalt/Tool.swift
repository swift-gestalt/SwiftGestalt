...
public class SwiftGestaltToolOptions {
  let verbosity: Int = Verbosity.concise.rawValue
  public required init() {}
}

public class SwiftGestaltTool<Options: SwiftGestaltToolOptions> {
  ...
  public init(toolName: String, usage: String, overview: String, args: [String],
    ...
    do {
      ...
      if let error = error as? ArgumentParserError {
        SwiftGestaltTool.exit(with: type(of: self).handleArgumentParserError(error))
      }
      SwiftGestaltTool.exit(with: .failure)
    }
  }
  ...
  public func run() {
    ...
    SwiftGestaltTool.exit(with: executionStatus)
  }
